...
- **han-kernel-crash-analyzer-v2** —— Linux 内核崩溃分析（高通/Android），覆盖 NULL 指针 / KASAN UAF / SLUB 损坏 / ABBA 死锁 / panic / ramdump。反汇编优先 + Agent 对抗验证 + 案例自学习闭环。
...
├── skills/
│   └── <name>/                  # 每个 skill 一个目录（han-skill-creator-plus + han-* 生图体系 + han-llm-wiki + han-kernel-crash-analyzer-v2 + han-d2-diagram + han-git-commit + han-flash-test），单一真源
...
仓库已内置 `.cursor/rules/han-skill-creator-plus.mdc`（`alwaysApply: false`），在对话里用 `@han-skill-creator-plus` 引用即可按需加载。改了 SKILL.md 后重新生成：
...
| `han-kernel-crash-analyzer-v2` | Linux 内核崩溃分析（高通/Android）：NULL 指针 / KASAN UAF / SLUB / ABBA 死锁 / panic / ramdump；反汇编优先 + 对抗验证 + 案例自学习 |
