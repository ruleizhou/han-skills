...
将 `llm-wiki` 目录复制到你的 Claude Code skills 目录：

```bash
# 方式一：复制到项目级
cp -r llm-wiki /你的项目目录/.claude/skills/
...
```
1. /llm-wiki init              → 创建 wiki 目录骨架
2. 把深度知识文档放到 note/10-Domains/ 下
3. /llm-wiki ingest            → 收录文档到 wiki（自动生成知识卡片）
4. /llm-wiki query 你的问题     → 查询知识库
5. /llm-wiki card              → 抽卡复习学习
6. /llm-wiki weekly            → 生成本周工作总结
7. /llm-wiki lint              → 定期健康检查
```
...
```
/llm-wiki query Scaling Laws 和语言模型有什么关系？
```
...
```
/llm-wiki card              # 随机抽取一张（智能优先：新学的、复习少的优先）
/llm-wiki card quiz         # 抽卡 + 测验模式（出选择题/判断题考你）
/llm-wiki card discuss      # 抽卡 + 讨论模式（围绕卡片展开深度讨论）
/llm-wiki card Scaling Laws # 抽取指定概念的卡片
```
...
执行 `/llm-wiki ingest` 时，每创建一个新的概念页面，都会自动生成对应的卡片。如果该概念已有卡片，新信息会被整合进去（补充要点、增加测验题）。

**三种复习模式：**

| 模式 | 命令 | 做什么 |
|------|------|--------|
| 浏览 | `/llm-wiki card` | 展示卡片核心要点，问你要测验还是讨论 |
| 测验 | `/llm-wiki card quiz` | 展示要点回顾 → 出 2-3 道题 → 判卷解析 → 更新掌握度 |
| 讨论 | `/llm-wiki card discuss` | 选一个切入点提问 → 引导你思考和表达 → 总结讨论收获 → 追加到卡片 |
...
```
/llm-wiki weekly            # 总结本周
/llm-wiki weekly 上周        # 总结上周
/llm-wiki weekly 2026-04-06 # 总结指定日期所在的那一周
```
...
你：/llm-wiki query Transformer 和 Scaling Laws 有什么关系？
Claude：[读取相关 wiki 页面，综合回答]
        Transformer 架构是 Scaling Laws 的载体...

你：/llm-wiki card quiz
Claude：抽到卡片「Scaling Laws」
        快速回顾：语言模型性能与规模呈幂律关系...
