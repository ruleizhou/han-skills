...
**触发条件：**
- `/llm-wiki init` — 扫描工作区，生成 scaffold 提案
...
6. **`wiki/.ingest-folders.yaml`**（格式见 cmd-ingest.md）
   - 💡 若用户已通过 `/add-dir` 明确加入源码目录（如内核源码），可预注册该目录（`last_ingest: null`），便于后续无参数 `/llm-wiki ingest` 直接收录
...
7. **`.raw/`**（vault 根目录，与 `00-Home/` 同级）— 原始文档收件箱
   ...
   - 自动将 `.raw/` 注册到 `wiki/.ingest-folders.yaml`（`last_ingest: null`），便于后续无参数 `/llm-wiki ingest` 直接收录
   - 若 `.raw/` 和注册条目均已存在 → 跳过（幂等）

   > **注意**：`.raw/` 已被注册到 `.ingest-folders.yaml`，后续运行无参数 `/llm-wiki ingest` 时会自动收录其中的新文档到 `wiki/sources/`。已在 wiki 目录中的文件自动算作已收录。
...
9. **`wiki/scripts/wiki-search.py`** — 同上（`<skill-dir>/scripts/wiki-search.py`，纯 Python stdlib，零依赖）
   > skill 目录 = 加载本 SKILL.md 的目录（含 `references/`、`scripts/`、`SKILL.md`）。若 `.claude/skills/llm-wiki` 是软链接，用 `realpath` 解析确认真实路径后再 `cp`，避免软链导致 `cp: cannot stat`。
