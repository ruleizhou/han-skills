...
V2 基于 han-skill-creator-plus 模板重构，V1 作为备份版本仅通过 `/kernel-crash-analyzer` 命令触发。
...
V2 基于 han-skill-creator-plus 的文件拆分 + 自学习架构模板重新生成。

### 与 V1 的关系

- V2 为默认版本（自动触发），V1 为备份（仅 `/kernel-crash-analyzer` 命令触发）
- V2 目录：`skills/han-kernel-crash-analyzer-v2/`
...
- SKILL.md 路由层严格遵循 han-skill-creator-plus 模板结构
