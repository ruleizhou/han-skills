...
> **活文档（如 `80-Notes/`）的增量语义——「新增即收」**：上述机制只判定「文件是否首次收录」。对用户持续增删改的活文档目录，无参数 ingest 只收其中**尚无来源页的新文件**；已收录文件的修改不会被自动重收，需显式 `/han-llm-wiki ingest 80-Notes` 触发重收（重收会整合新内容进既有概念/实体页，不删除旧产物）。
...
**Step 4.6（图表生成）的模式感知**：`attachment/` 目录建在页面**实际所在目录**下（按上方路由表）。例如 engineering 模式概念页在 `wiki/concepts/`，图就在 `wiki/concepts/attachment/`；lyt 模式页面在 `wiki/notes/`，图就在 `wiki/notes/attachment/`。
...
> **默认预注册源**：init 会把 `80-Notes/`（用户笔记区）和 `.raw/`（原始收件箱）默认注册到 `.ingest-folders.yaml`。无参数 ingest 时，它们与其他已注册目录一起被增量收录（`80-Notes/` 按「新增即收」，见上文「活文档增量语义」）。
...
3. **无参数时：**
   ...
   - 文件不存在或 `folders` 为空 → **进入自动发现流程**：
     1. 扫描 vault 根目录下除 `00-Home/`~`90-templates/`、`wiki/`、`.raw/`、`.claude/`、`assets/`、`Temp/` 外的所有一级目录，标记为候选收录源
...
3. 对每个待配图页面（按引擎执行，命令见 diagram-guide.md 第 4 节）：
   - 在页面**同目录**建 `attachment/`（如 `wiki/concepts/attachment/`，路径随 mode 路由表）
   - **D2**：读 `han-d2-diagram/assets/templates/<type>.d2` 作骨架，写 `<页面slug>-<短码>.d2` → 编译 SVG
   - **AI 三件套**：跑对应 skill 的 workflow 生成 prompt → 调 `han-imagen --json` 拿 output_path → `cp` 到 `attachment/<slug>-<code>.png`
   - 按图类型嵌入（位置规则见 diagram-guide §5）：信息图前置到引言下方，结构图入对应章节；`![图 N: 标题](attachment/xxx.svg|.png)` + 图号 + **图后 2-5 句说明**
4. **验证门（必须执行）**：对每张图 `ls -lh attachment/<name>` 确认文件存在且非空；确认嵌入的相对路径正确（指向页面同目录的 `attachment/`）
...
3. **步骤 4.6 — 为内容页生成图表（多引擎图文并茂）**：按 `references/diagram-guide.md` 智能选引擎为本次新建/更新的概念页 + 来源页配图（实体页/卡片不配）。因要写 `attachment/` 文件并修改多个页面，**必须在主 agent 串行执行**（不下放给并行 sub-agent）
