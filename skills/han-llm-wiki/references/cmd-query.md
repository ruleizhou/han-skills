# /han-llm-wiki query —— 查询

基于 wiki 内容回答用户的问题。采用三阶段混合检索管线。

**触发条件：**
- 用户显式执行 `/han-llm-wiki query`
- 用户向 wiki 提问，如「帮我解释一下」「wiki 里有什么关于...」
- `/han-llm-wiki query --think` → 在回答前先过 think 循环，确保深度审视
...
4. **建议存页**——如果查询产生了有价值的分析，主动建议 `/han-llm-wiki save <标题>` 归档到 `wiki/analyses/`
