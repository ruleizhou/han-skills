...
- **crash 加载**（必带 `--kaslr`，见 `workflows/crash-parse-raw.md`「⚠️ 强制前置检查」）：`crash <vmlinux> "DDRCS0_0.BIN@0x40000000,DDRCS1_0.BIN@0xc0000000" --kaslr=<OCIMEM 0xdead4ead现取> --no_data_debug --machdep vabits_actual=39 --no_panic`。KASLR 每次启动随机，**必须现取不可复用**（案 elo/5#=0x1160000000、案 115706=0x2617200000 各不同）。**勿用** `--machdep kimage_voffset`（segfault）
