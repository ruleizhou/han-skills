...
description: >
  高通/Android 平台内核崩溃 dump 根因分析（kernel panic、ramdump、死机、KASAN
  UAF、SLUB 损坏、ABBA 死锁），提供 parser_out/symbols/kasan.txt 即进分析模式；
  「修好了/根因确认了」进反馈闭环。
  无 dump 的重启行为异常用 qualcomm-reboot-analyzer。
---
...
- **动态复现验证（Step 8B 调用）**：Skill `diagnosing-bugs` — 偶发/竞态类 crash 建复现回路 + 回归测试的 P1-P5 纪律，Step 8B 借用
