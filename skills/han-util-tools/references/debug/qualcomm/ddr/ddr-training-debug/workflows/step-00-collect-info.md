...
用 **一次 `结构化提问`（多问题）** 一次性收集 DDR 训练失败的全部必要信息，禁止分多次询问。

## 0.1 问题列表（合并为单次 结构化提问）
...
> **多选规则**：四个问题合并为一次 `结构化提问` 调用，`multiSelect: false`，四个问题各一个 `header`。注意 结构化提问 最多 4 个选项，源码路径第 4 题需要拆分处理（见 0.1.1）。
