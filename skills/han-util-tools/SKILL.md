---
name: han-util-tools
description: "内核工具总路由，仅通过 /han-util-tools 命令触发。按 场景→平台→模块→类型 四级交互引导，覆盖 Debug / 器件 Bringup / 功能开发 三大场景，定位到对应子 skill。"
---
...
1. **交互选择不可跳过（最高优先级）** — 扫描完成后唯一合法动作是调用 结构化提问，严禁推断用户意图
...
5. **新建子 skill 统一委托 han-skill-creator-plus** — 自动获得拆分架构 + 自学习能力
...
| `../han-skill-creator-plus/` | 子 skill 创建工具（统一委托） |
